...
:point_right: Remarquez comment il est possible de donner une valeur par défaut à `$subject` en passant un second argument à `$this->app['request']->request->get()`
...
:point_right: A noter l’utilisation de la méthode `redirectToRoute()` du contrôleur de base pour réaliser une redirection vers une route donnée.
